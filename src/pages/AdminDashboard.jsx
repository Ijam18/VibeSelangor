...
export default function AdminDashboard({
    ...
    const handleExportPDF = () => {
        ...
        const reportProfiles = (filteredProfiles || []).filter((p) => !['owner', 'admin'].includes((p?.role || '').toLowerCase()));
        const certMap = new Map((allCertificates || []).map((c) => [c.builder_id, c]));
        const normalizeDistrict = (value) => String(value || '').trim().toLowerCase();
        const seededDistricts = Array.from(
            new Set([
                ...(DISTRICT_OPTIONS || []),
                ...reportProfiles.map((p) => p?.district).filter(Boolean)
            ])
        );
        ...
        const completionRate = reportProfiles.length > 0
            ? Math.round((projectsSubmittedInPeriod / reportProfiles.length) * 100)
            : 0;
            ...
            <>
                <MobileFeatureShell title="Admin Dashboard" subtitle="Ops cockpit" islandContent={<span style={{ fontSize: 10, fontWeight: 600 }}>{islandMessages[islandIndex]}</span>} onNavigate={setPublicPage}>
                    <div style={{ display: 'grid', gap: 10 }}>
                        ...
                        </section><section style={{ borderRadius: 14, border: '1px solid rgba(148,163,184,0.35)', background: 'rgba(255,255,255,0.78)', padding: '10px 11px', overflow: 'hidden' }}>
    ...
    return (
        <div className="container" style={{ padding: '40px 24px' }}>
            ...
            <div style={{ display: 'flex', gap: '20px', marginBottom: '24px', borderBottom: '2px solid #eee' }}>
                ...
                <button onClick={() => setAdminTab('reports')} style={{ padding: '12px 12px', border: 'none', background: 'none', borderBottom: adminTab === 'reports' ? '4px solid var(--selangor-red)' : '44px solid transparent', fontWeight: '900', fontSize: '13px', cursor: 'pointer', color: adminTab === 'reports' ? 'black' : '#888', transition: 'all 0.2s' }}>REPORTS</button>
            </div>
